...
export default async function BalanceSheetPage({ searchParams }: Props) {
  ...
  return (
    <div>
      ...
      <div className="mt-8 border border-line-strong p-6 flex items-baseline justify-between">
        <div>
          ...
          <tbody>
            {lines.length === 0 ? (
              <tr className="border-t border-line">
                <td colSpan={3} className="px-4 py-3 text-ink-muted italic">
                  No balances.
                </td>
              </tr>
            ) : (
              lines.map((l) => (
                <tr key={l.account_id} className="border-t border-line">
                  <td className="px-4 py-3 text-ink-muted">{l.code}</td>
