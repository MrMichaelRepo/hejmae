...
export default function LedgerPage() {
  ...
  const [fullAccounts, setFullAccounts] = useState<AccountRow[]>([])
  ...
  // Projects + full account rows feed the manual-entry modal. The ledger
  // endpoint returns a narrowed account projection (no is_active flag), so
  // we hit /accounts separately for the editor.
  useEffect(() => {
    Promise.all([
      ...
      setFullAccounts((a.data as AccountRow[]) ?? [])
    })
  }, [])
  ...
  const accountIx = useMemo(() => {
    if (!data) return new Map<string, LedgerResponse['accounts'][number]>()
